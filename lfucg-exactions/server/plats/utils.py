...
def calculate_lot_balance(lot):
    # PAYMENTS
    payments = Payment.objects.exclude(is_active=False, is_approved=False).filter(lot_id=lot.id)
    ...
    # ACCOUNT LEDGERS
    account_ledgers = AccountLedger.objects.exclude(is_active=False, is_approved=False).filter(lot=lot.id, entry_type='USE')
