...
class Command(BaseCommand):
  ...
  def handle(self, *args, **options):
    ...
    jan_nine = AccountLedger.objects.filter(
      ...
      lot__address_street__icontains='Sweet Clover',
    ).exclude(
      lot__address_full__icontains='3348 Sweet Clover Park',
      is_active=False,
    )
    ...
    for ledger in jan_nine:
      print('JAN DATE LEDGER', ledger)
      date_hist = ledger.history.order_by('-date_modified').distinct('date_modified')
      print('DATE HIST ', date_hist)

      nine_index = [l.date_modified for l in date_hist].index(datetime.date(2020, 1, 27))
